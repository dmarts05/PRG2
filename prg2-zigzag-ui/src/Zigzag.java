...
public class Zigzag {
	 ...
	 */
	private ArrayList<Integer> zigzagNumbers = new ArrayList<Integer>();
		...
		while (true) {
			...
			if (board[rows - 1][columns - 1].equals(currentNode)) {
				...
				if (getNodeFromCoordinates(currentNode.getLinkCoordinates()) != null) {
					...
					previousNode.addRestrictedNodeCoordinates(currentNode.getCoordinates()[0],
							currentNode.getCoordinates()[1]);
	...
	private int getLinkTypeToValidNode(Node currentNode) {
		...
		for (int i = 1; i <= 9; i++) {
			...
			if (isNextNumber(currentNode, nextNode)
					&& !currentNode.areNodeCoordinatesRestricted(nextNode.getCoordinates()[0],
							nextNode.getCoordinates()[1])
	 ...
	 */
	public void linkNodes(Node currentNode, Node nextNode, int linkType) {
	...
	public void unlinkNodes(Node currentNode, Node previousNode) {
	...
	public int tryGetLinkType(Node currentNode, Node nextNode) {
		int[] currentNodeCoordinates = currentNode.getCoordinates();
		int currentNodeRow = currentNodeCoordinates[0];
		int currentNodeColumn = currentNodeCoordinates[1];

		int nextNodeLinkType = -1;
		...
		for (int i = 1; i <= 9; i++) {
			switch (i) {
				...
				case 1:
					// Upper left
					try {
						if (board[currentNodeRow - 1][currentNodeColumn - 1].equals(nextNode)) {
							// Set nextNode's linkType to lower right
							nextNodeLinkType = 9;
						} else {
							continue;
				...
				case 2:
					// Upper
					try {
						if (board[currentNodeRow - 1][currentNodeColumn].equals(nextNode)) {
							// Set nextNode's linkType to lower
							nextNodeLinkType = 8;
						} else {
							continue;
				...
				case 4:
					// Left
					try {
						if (board[currentNodeRow][currentNodeColumn - 1].equals(nextNode)) {
							// Set nextNode's linkType to right
							nextNodeLinkType = 6;
						} else {
							continue;
	...
	public boolean trySetLink(Node currentNode, Node nextNode) {
		int nextNodeLinkType = tryGetLinkType(currentNode, nextNode);
		if (nextNodeLinkType == -1) {
			return false;
		}
